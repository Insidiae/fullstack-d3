function drawHistogram(data, metric) {
  //* Step 1. Access data
  ...
  const binGroups = allBinsGroup
    ...
    .attr(
      "aria-label",
      (bin) =>
        `There were ${yAccessor(bin)} days with ${metric} between ${
          bin.x0
        } and ${bin.x1}.`
    );
...
async function drawBars() {
  ...
  //* Extra Credit: Draw multiple histograms, one for each metric
  const metrics = [
    "windSpeed",
    "moonPhase",
    "dewPoint",
  ...
  metrics.forEach((metric) => drawHistogram(data, metric));
}
drawBars();
