async function drawHistogram() {
  ...
  const xAccessor = (d) => parseInt(d["num_colors"]);
  ...
  wrapper
    ...
    .append("title")
    .text(
      `Histogram looking at the distribution of unique colors of Bob Ross paintings`
    );
  ...
  const binGroups = allBinsGroup
    ...
    .attr(
      "aria-label",
      (bin) =>
        `There were ${yAccessor(bin)} Bob Ross paintings with ${
          bin.x0
        } unique colors.`
    );
  ...
  const xAxisLabel = xAxis
    ...
    .text("Number of unique colors used in a Bob Ross painting");
}
